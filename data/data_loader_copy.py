...
class DistributedBucketingSampler(Sampler):
    def __init__(self, data_source, batch_size=1, num_replicas=None, rank=None):
        ...
        self.bins = [self.ids[i:i + batch_size] for i in range(0, len(self.ids), batch_size)] #ID LIST GROUPS OF TOTAL NUMBER OF BATCHES IN THE DATASET (TRAIN,TEST,VAL)
        self.num_replicas = num_replicas
        self.rank = rank
        self.num_samples = int(math.ceil(len(self.bins) * 1.0 / self.num_replicas)) #Total number of samples to be drawn for each world.(here replicas)
        self.total_size = self.num_samples * self.num_replicas #total number of samples to be drawn 

    def __iter__(self):
        offset = self.rank
        # add extra samples to make it evenly divisible
        bins = self.bins + self.bins[:(self.total_size - len(self.bins))] #appending the first "(total_size - number of bins)", bins to the bins again so that equal number of partitions can be made in the total number of bins. 
