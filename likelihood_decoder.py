...
class BeamCTCDecoder(Decoder):
    ...
    def convert_to_transcripts(self, out):
        results =[]
        for _,batch in enumerate(out):
            transcript = [''.join(map(lambda x: self.int_to_char[x.item()],batch[0:len(batch)]))]
    ...
    def decode(self, probs, sizes=None):
        ...
        #print(out)
        #print(offsets)
        #print('entered decode')
        strings = self.convert_to_strings(out, seq_lens)
        offsets = self.convert_tensor(offsets, seq_lens)
        return strings, offsets, scores, seq_lens
