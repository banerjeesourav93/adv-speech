    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        with torch.no_grad():
            for i, (data) in tqdm(enumerate(test_loader), total=len(test_loader)):
                ...
                val_loss= criterion(out, targets, output_sizes, target_sizes).to(device)		
                val_loss = val_loss / inputs.size(0)
                if args.distributed:
                    val_loss_value = reduce_tensor(val_loss, args.world_size).item()
                else:
                    val_loss_value = loss.item()
                validation_losses.append(val_loss_value)
 
                out = out.transpose(0,1) #retranspose it all to make decodings
